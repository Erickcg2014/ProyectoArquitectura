...
public class OrdenIntegration {
    ...
    public void producirEventoOrderPaid(UUID idOrden) {
        String topic = "topico_ordenes";
        String message = "{\"event\": \"OrderPaid\", \"orderId\": \"" + idOrden + "\"}";
        kafkaTemplate.send(topic, message);
        System.out.println("Evento OrderPaid enviado a Kafka topic 'topico_ordenes': " + message);
    }

    // Producir evento OrderStatusUpdate
    public void producirEventoOrderStatusUpdate(UUID idOrden, UUID idUsuario, String status) {
        String topic = "topico_ordenes";
        String message = String.format(
            "{\"orderId\": \"%s\", \"userId\": \"%s\", \"status\": \"%s\", \"timestamp\": \"%s\"}",
            idOrden, idUsuario, status, java.time.Instant.now().toString()
        );
        kafkaTemplate.send(topic, message);
        System.out.println("Evento OrderStatusUpdate enviado a Kafka topic 'topico_ordenes': " + message);
    }
