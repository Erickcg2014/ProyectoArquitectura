...
public class OrdenService {
    ...
    public void marcarComoEnviada(UUID idOrden) {
        ...
        // Obtener la orden para el userId
        Orden orden = ordenRepository.findById(idOrden).orElse(null);
        if (orden != null) {
            ordenIntegration.producirEventoOrderStatusUpdate(idOrden, orden.getIdUsuario(), "enviado");
        }
    }

    @Transactional
    public void marcarComoAlistando(UUID idOrden) {
        // Cambiar estado ALISTANDO
        ordenRepository.updateEstadoById(idOrden, "ALISTANDO");
        // Obtener la orden para el userId
        Orden orden = ordenRepository.findById(idOrden).orElse(null);
        if (orden != null) {
            ordenIntegration.producirEventoOrderStatusUpdate(idOrden, orden.getIdUsuario(), "alistando");
        }
    }

    @Transactional
    public void marcarComoEntregadoTransportadora(UUID idOrden) {
        // Cambiar estado ENTREGADO_TRANSPORTADORA
        ordenRepository.updateEstadoById(idOrden, "ENTREGADO_TRANSPORTADORA");
        // Obtener la orden para el userId
        Orden orden = ordenRepository.findById(idOrden).orElse(null);
        if (orden != null) {
            ordenIntegration.producirEventoOrderStatusUpdate(idOrden, orden.getIdUsuario(), "entregado_transportadora");
        }
    }

    @Transactional
    public void marcarComoEntregado(UUID idOrden) {
        // Cambiar estado ENTREGADO
        ordenRepository.updateEstadoById(idOrden, "ENTREGADO");
        // Obtener la orden para el userId
        Orden orden = ordenRepository.findById(idOrden).orElse(null);
        if (orden != null) {
            ordenIntegration.producirEventoOrderStatusUpdate(idOrden, orden.getIdUsuario(), "entregado");
        }
    }
